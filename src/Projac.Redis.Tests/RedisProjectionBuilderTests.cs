...
{
    [TestFixture]
    public class RedisProjectionBuilderTests
    {
        private RedisProjectionBuilder _sut;
        ...
        {
            Assert.Throws<ArgumentNullException>(() => new RedisProjectionBuilder(null));
        }
        ...
        {
            Assert.Throws<ArgumentNullException>(() => _sut.When<object>((Func<ConnectionMultiplexer, object, Task>)null));
        }

        [Test]
        public void WhenHandlerWithTokenCanNotBeNull()
        {
            Assert.Throws<ArgumentNullException>(() => _sut.When<object>((Func<ConnectionMultiplexer, object, CancellationToken, Task>)null));
        }
        ...
        public void WhenHandlerReturnsExpectedResult()
        {
            ...
            Assert.That(result, Is.InstanceOf<RedisProjectionBuilder>());
        }
        ...
        public void WhenHandlerIsPreservedUponBuild()
        {
            ...
            Func<ConnectionMultiplexer, object, Task> handler = (connection, message) => task;
        ...
        {
            var task = Task.FromResult(false);
            Func<ConnectionMultiplexer, object, CancellationToken, Task> handler = (connection, message, token) => task;
            var result = _sut.When<object>(handler).Build();
        ...
        public void WhenHandlerPreservesPreviouslyCollectedHandlersUponBuild()
        {
            ...
            Func<ConnectionMultiplexer, object, Task> handler1 = (connection, message) => task1;
            Func<ConnectionMultiplexer, object, Task> handler2 = (connection, message) => task2;
        ...
        public void WhenHandlerWithTokenPreservesPreviouslyCollectedHandlersUponBuild()
        {
            var task1 = Task.FromResult(false);
            var task2 = Task.FromResult(false);
            Func<ConnectionMultiplexer, object, CancellationToken, Task> handler1 = (connection, message, token) => task1;
            Func<ConnectionMultiplexer, object, CancellationToken, Task> handler2 = (connection, message, token) => task2;
            var result = _sut.When<object>(handler1).When<object>(handler2).Build();
