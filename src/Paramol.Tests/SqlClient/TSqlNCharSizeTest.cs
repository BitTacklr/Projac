...
{
    [TestFixture]
    public class TSqlNCharSizeTest
    {
        [Test]
        public void MaxReturnsExpectedResult()
        {
            Assert.That(TSqlNCharSize.Max, Is.EqualTo(new TSqlNCharSize(-1)));
        }
        ...
        {
            var sut = SutFactory();
            Assert.That(sut, Is.InstanceOf<IEquatable<TSqlNCharSize>>());
        }
        ...
        {
            TSqlNCharSize sut = 123;

            Assert.That(sut, Is.EqualTo(SutFactory(123)));
        }

        private static TSqlNCharSize SutFactory()
        {
            return SutFactory(new Random().Next(-1, 4000));
        }

        private static TSqlNCharSize SutFactory(int value)
        {
