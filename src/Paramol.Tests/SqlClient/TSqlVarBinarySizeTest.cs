...
{
    [TestFixture]
    public class TSqlVarBinarySizeTest
    {
        [Test]
        public void MaxReturnsExpectedResult()
        {
            Assert.That(TSqlVarBinarySize.Max, Is.EqualTo(new TSqlVarBinarySize(-1)));
        }
        ...
        {
            var sut = SutFactory();
            Assert.That(sut, Is.InstanceOf<IEquatable<TSqlVarBinarySize>>());
        }
        ...
        {
            TSqlVarBinarySize sut = 123;

            Assert.That(sut, Is.EqualTo(SutFactory(123)));
        }

        private static TSqlVarBinarySize SutFactory()
        {
            return SutFactory(new Random().Next(-1, 8000));
        }

        private static TSqlVarBinarySize SutFactory(int value)
        {
