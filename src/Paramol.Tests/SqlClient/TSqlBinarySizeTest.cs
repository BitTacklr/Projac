...
{
    [TestFixture]
    public class TSqlBinarySizeTest
    {
        ...
        {
            var sut = SutFactory();
            Assert.That(sut, Is.InstanceOf<IEquatable<TSqlBinarySize>>());
        }
        ...
        {
            TSqlBinarySize sut = 123;

            Assert.That(sut, Is.EqualTo(SutFactory(123)));
        }

        private static TSqlBinarySize SutFactory()
        {
            return SutFactory(new Random().Next(-1, 8000));
        }

        private static TSqlBinarySize SutFactory(int value)
        {
