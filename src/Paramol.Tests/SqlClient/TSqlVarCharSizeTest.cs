...
{
    [TestFixture]
    public class TSqlVarCharSizeTest
    {
        [Test]
        public void MaxReturnsExpectedResult()
        {
            Assert.That(TSqlVarCharSize.Max, Is.EqualTo(new TSqlVarCharSize(-1)));
        }
        ...
        {
            var sut = SutFactory();
            Assert.That(sut, Is.InstanceOf<IEquatable<TSqlVarCharSize>>());   
        }
        ...
        {
            TSqlVarCharSize sut = 123;

            Assert.That(sut, Is.EqualTo(SutFactory(123)));
        }

        private static TSqlVarCharSize SutFactory()
        {
            return SutFactory(new Random().Next(-1, 8000));
        }

        private static TSqlVarCharSize SutFactory(int value)
        {
